from typing import Optional, Sequence

from .database import Database
from .entity import Genre


class GenreData:

    def __init__(self, db: Database):
        self._db = db

    def list(self) -> Sequence[Genre]:
        sql = """
            SELECT id_genre, name, description
            FROM genre
            ORDER BY id_genre;
        """
        rows = self._db.fetch_all(sql)
        return [Genre(*row) for row in rows]

    def get(self, id_genre: int) -> Optional[Genre]:
        sql = """
            SELECT id_genre, name, description
            FROM genre
            WHERE id_genre=%s;
        """
        row = self._db.fetch_one(sql, id_genre)
        return Genre(*row) if row else None

    def add(self, name: str, description: str) -> None:
        sql = """
            INSERT INTO genre (name, description)
            VALUES (%s, %s);
        """
        self._db.execute(sql, name, description)

    def delete(self, id_genre: int) -> None:
        sql = """
            DELETE FROM genre
            WHERE id_genre=%s;
        """
        self._db.execute(sql, id_genre)

    def update(self, id_genre: int, name: str, description: str) -> None:
        sql = """
            UPDATE genre
            SET name=%s, description=%s
            WHERE id_genre=%s;
        """
        self._db.execute(sql, name, description, id_genre)
