...
class CassetteRepository:
    ...
    def list(self) -> Sequence[Cassette]:
        sql = """
            ...
            FROM cassette c
            LEFT JOIN cassette_genre cg
                ON cg.id_cassette = c.id_cassette
            LEFT JOIN genre g
                ON g.id_genre = cg.id_genre
    ...
    def list_available(self) -> Sequence[Cassette]:
        sql = """
            ...
            LEFT JOIN genre g
                ON g.id_genre = cg.id_genre
            LEFT JOIN rental r
                ON r.id_cassette = c.id_cassette
                AND r.status = 'В процессе'
    ...
    def __fetch_with_genres(self, sql: str, *params):
